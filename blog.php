...
class BlogDBLoader implements IBlogLoader{
    ...
    function load(String $path):array
    {
        ...
        $select = $this->pdo->query('SELECT * FROM '.$path.' INNER JOIN auteur ON auteur.id');
        while($data = $select->fetch(PDO::FETCH_ASSOC)){
             array_push($this->dbname, $data);
        }

        $auteurs = array_map(function($auteur){
            return new Author($auteur{'id'}, $auteur{'firstname'}, $auteur{'lastname'});
        },$this->dbname);
        $articles = array_map(function($article)use($auteurs){
            $idAuteurs = $article{'idAuteur'};
            ...
            $auteurDarticle = current($auteurDarticle);
                    return new Article(
                            $article{'id'}, $article{'title'},$article{'contenu'},
                        $auteurDarticle, new DateTime($article['date'])
            );
        },$this->dbname);
        return $articles;
    }
...
$articlesLoad = new BlogDBLoader('localhost','katoo','root','M+D=cdna4');
